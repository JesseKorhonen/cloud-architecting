...
export class StudentService {
  private apiUrl = 'http://localhost:3000/student'; // apin osoite
  ...
  */
  ...
  addStudent(student: any): Observable<Student> {
    ...
    return this.http
      .post<Student>(this.apiUrl, student, headers)
      .pipe(catchError(this.handleError));
  }
  ...
  updateStudent(student: any): Observable<Student> {
    const mytoken = JSON.parse(sessionStorage['accesstoken']);
    student.token = mytoken.token; // pistetään token bodyn mukana
    const url = `${this.apiUrl}/${student.studentcode}`;
    return this.http
      .put<Student>(url, student, headers)
      .pipe(catchError(this.handleError));
  }
  ...
  delStudent(id: string): Observable<Student> {
    ...
    const url = `${this.apiUrl}/${id}`;
    return this.http
      .delete<Student>(url, tokenheaders)
      .pipe(catchError(this.handleError));
  }
}
